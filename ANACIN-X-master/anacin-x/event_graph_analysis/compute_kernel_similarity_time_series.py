...
def compute_kernel_similarity_matrices( slice_subgraphs, kernel_params ):
    ...
    for kp in kernel_params:
        ...
        # Compute VertexEdgeHist
            ...
            similarity_mat = gk.CalculateVertexEdgeHistKernel( relabeled_graphs )
            kernel_to_distance_matrix[ key ] = similarity_mat
        # Compute VertexHistGauss
        elif kernel == "vhgauss":
            similarity_mat = gk.CalculateVertexHistGaussKernel( relabeled_graphs )
            kernel_to_distance_matrix[ key ] = similarity_mat
        # Compute EdgeHistGauss
        elif kernel == "ehgauss":
            similarity_mat = gk.CalculateEdgeHistGaussKernel( relabeled_graphs )
            kernel_to_distance_matrix[ key ] = similarity_mat
        # Compute KStepRandomWalk
        elif kernel == "ksrwalk":
            similarity_mat = gk.CalculateKStepRandomWalkKernel( relabeled_graphs )
            kernel_to_distance_matrix[ key ] = similarity_mat
        else:
