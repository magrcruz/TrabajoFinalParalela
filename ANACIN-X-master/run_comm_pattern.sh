...
#done
#ndp_step_count=$(echo "scale=1; $nd_end/$nd_iter - $nd_start/$nd_iter" |bc -l)
#echo "Step Counter is: ${ndp_step_count}"
...
if [ ${comm_pattern} == "message_race" ]; then
    ...
    kdts_file=${results_path}/msg_size_${msg_sizes}/n_procs_${n_procs}/n_iters_${n_iters}/ndp_${nd_start}_${nd_iter}_${nd_end}/kdts.pkl
    ...
    kdts_file=${results_path}/msg_size_${msg_sizes}/n_procs_${n_procs}/n_iters_${n_iters}/ndp_${nd_start}_${nd_iter}_${nd_end}/proc_placement_pack/nd_topological_${nd_topo}/kdts.pkl
fi
...
#python3 anacin-x/event_graph_analysis/visualization/make_message_nd_plot.py ${kdts_file} ${comm_pattern} ${graph_kernel} ${results_path}/../kdts_vis_${comm_pattern} ${nd_start} ${nd_iter} ${nd_end} --nd_neighbor_fraction ${nd_topo}
